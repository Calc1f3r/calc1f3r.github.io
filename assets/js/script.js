...
(function () {
  ...
  function show(targetLabel) {
    ...
    // Marca pestaña activa + accesibilidad
    navLinks.forEach((btn) => {
      const isActive =
        btn.textContent.trim().toLowerCase() === target;
  ...
  // Click en tabs
  navLinks.forEach((btn) => {
    btn.addEventListener('click', () => show(btn.textContent));
  });

  // Estado inicial (si no hay .active, cae a 'about')
